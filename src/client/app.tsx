...
import { getCategories, getTransactionPage, getVisibleTransactions, transactionsPerPage, type SortDirection, type SortField } from './transaction-list';
...
function Dashboard({ result, reset }: { result: Result; reset: () => void }) {
  ...
  const transactionPage = getTransactionPage(visibleTransactions, page);
  ...
  return <main className="app-shell min-h-svh"><div className="mx-auto max-w-6xl px-6 py-9 md:py-12"><header className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-start"><div><Badge variant="secondary" className="mb-3">Результат анализа</Badge><h1 className="text-4xl font-bold tracking-tight">Ваши расходы</h1><p className="mt-2 text-muted-foreground">{result.period.from} — {result.period.to}</p></div><div className="flex items-center gap-2"><ThemeToggle /><Button variant="outline" onClick={reset}><FileText />Новая выписка</Button></div></header>
    ...
    <Card><CardHeader><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><CardTitle>Операции</CardTitle><div className="flex items-center gap-2"><label className="flex h-8 items-center gap-2 rounded-md border bg-background px-2 text-sm shadow-xs"><ListFilter className="size-3.5 text-muted-foreground" aria-hidden="true" /><span className="sr-only">Фильтр по категории</span><select value={categoryFilter} onChange={(event) => { setCategoryFilter(event.target.value); setPage(1); }} className="transaction-category-filter min-w-34 bg-transparent text-foreground outline-none"><option value="">Все категории</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select></label>{categoryFilter && <Button variant="ghost" size="sm" onClick={() => { setCategoryFilter(''); setPage(1); }}><X />Сбросить</Button>}</div></div><CardDescription>{categoryFilter ? `Найдено: ${visibleTransactions.length} из ${result.transactions.length}` : `Всего операций: ${result.transactions.length}`}</CardDescription></CardHeader><CardContent className="px-2 sm:px-6"><Table className="table-fixed"><TableHeader><TableRow><TableHead className="w-20" aria-sort={sortField === 'date' ? sortDirection : 'none'}><SortButton field="date" label="Дата" /></TableHead><TableHead>Описание</TableHead><TableHead className="hidden w-32 sm:table-cell" aria-sort={sortField === 'category' ? sortDirection : 'none'}><SortButton field="category" label="Категория" /></TableHead><TableHead className="w-28 text-right" aria-sort={sortField === 'amount' ? sortDirection : 'none'}><span className="inline-flex"><SortButton field="amount" label="Сумма" /></span></TableHead></TableRow></TableHeader><TableBody>{transactionPage.transactions.length ? transactionPage.transactions.map((item, index) => <TableRow key={`${item.date}-${item.merchant}-${(transactionPage.currentPage - 1) * transactionsPerPage + index}`}><TableCell>{item.date}</TableCell><TableCell className="max-w-0"><div className="truncate" title={item.merchant}>{item.merchant}</div><Badge variant="secondary" className="mt-1 sm:hidden">{item.category ?? 'Не определена'}</Badge></TableCell><TableCell className="hidden sm:table-cell"><Badge variant="secondary">{item.category ?? 'Не определена'}</Badge></TableCell><TableCell className={`text-right font-medium ${item.type === 'expense' ? 'text-destructive' : 'text-emerald-600'}`}>{item.type === 'expense' ? '−' : '+'}{money.format(item.amount)}</TableCell></TableRow>) : <TableRow><TableCell colSpan={4} className="py-8 text-center text-muted-foreground">Операций в этой категории нет.</TableCell></TableRow>}</TableBody></Table>{transactionPage.totalPages > 1 && <nav className="mt-4 flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between" aria-label="Пагинация операций"><p className="text-sm text-muted-foreground">Показаны {(transactionPage.currentPage - 1) * transactionsPerPage + 1}–{Math.min(transactionPage.currentPage * transactionsPerPage, visibleTransactions.length)} из {visibleTransactions.length}</p><div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={() => setPage((value) => value - 1)} disabled={transactionPage.currentPage === 1}>Назад</Button><span className="min-w-24 text-center text-sm tabular-nums" aria-live="polite">Страница {transactionPage.currentPage} из {transactionPage.totalPages}</span><Button variant="outline" size="sm" onClick={() => setPage((value) => value + 1)} disabled={transactionPage.currentPage === transactionPage.totalPages}>Вперёд</Button></div></nav>}</CardContent></Card>
  </div></main>;
}
