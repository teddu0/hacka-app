...
function Dashboard({ result, reset }: { result: Result; reset: () => void }) {
  return <main className="mx-auto max-w-6xl px-6 py-9 md:py-12"><header className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-start"><div><Badge variant="secondary" className="mb-3">Результат анализа</Badge><h1 className="text-4xl font-bold tracking-tight">Ваши расходы</h1><p className="mt-2 text-muted-foreground">{result.period.from} — {result.period.to}</p></div><Button variant="outline" onClick={reset}><FileText />Новая выписка</Button></header>
    ...
    <Card><CardHeader><CardTitle>Операции</CardTitle></CardHeader><CardContent className="px-2 sm:px-6"><Table className="table-fixed"><TableHeader><TableRow><TableHead className="w-20">Дата</TableHead><TableHead>Описание</TableHead><TableHead className="hidden w-32 sm:table-cell">Категория</TableHead><TableHead className="w-28 text-right">Сумма</TableHead></TableRow></TableHeader><TableBody>{result.transactions.map((item, index) => <TableRow key={`${item.date}-${item.merchant}-${index}`}><TableCell>{item.date}</TableCell><TableCell className="max-w-0"><div className="truncate" title={item.merchant}>{item.merchant}</div><Badge variant="secondary" className="mt-1 sm:hidden">{item.category ?? 'Не определена'}</Badge></TableCell><TableCell className="hidden sm:table-cell"><Badge variant="secondary">{item.category ?? 'Не определена'}</Badge></TableCell><TableCell className={`text-right font-medium ${item.type === 'expense' ? 'text-destructive' : 'text-emerald-600'}`}>{item.type === 'expense' ? '−' : '+'}{money.format(item.amount)}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card>
  </main>;
}
